...
<div class="container">
    <div class="overlay">
        <form id="form-create-singer" action="/admin/singers?action=create" method="post">
            <div>
...
<script>
    ...
    function validateFormCreateSinger(){
        ...
        if (!name || name.trim().length <= 0  ){
            result = false;
            document.getElementById("error-name").innerHTML = "Tên bài hát không được để trống"
        }
        if ( name.trim().length >50){
            result = false;
            document.getElementById("error-name").innerHTML = "Tên nhạc sĩ  không được quá 50 ký tự"
        }
        if (!validateFullName(name)){
            result = false;
            document.getElementById("error_name").innerHTML = "tên nhạc sĩ không hợp lệ"
        }
        return result;
    }
</script>
