...
<div class="container">
    <div class="overlay">
        <form id="form-edit-category" action="/admin/categorys?action=edit&id=${category.id}" method="post">
            <div>
...
<script>
    ...
    function validateFromEditCategory() {
        ...
        if (!name || name.trim().length <= 0 || name.trim().length >50){
            result = false;
            document.getElementById("error_name").innerHTML = "Thể loại nhac không được để trống"
        }
        ...
        if (!validateFullName(name)){
            result = false;
            document.getElementById("error_name").innerHTML = "Thể loại nhac không được có ký tự đặc biệt hoặc số"
        }
        return result;
    }
</script>
